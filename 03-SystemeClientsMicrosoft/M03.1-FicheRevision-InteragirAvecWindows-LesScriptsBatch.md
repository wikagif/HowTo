...
### Définition
Nettoie le terminal sans fermer la fenêtre du terminal.

### Explication détaillée
La commande `cls /b` fonctionne comme `cls` mais sans fermer la fenêtre du terminal, ce qui permet de garder la session ouverte tout en effaçant le contenu affiché.

### Exemple
@echo off
cls /b
echo Le terminal a été nettoyé sans être fermé.
pause

### Exercice pratique
- Créez un script batch qui utilise `cls /b` à différentes étapes pour maintenir la session active.
